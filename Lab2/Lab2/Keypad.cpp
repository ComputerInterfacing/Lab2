...
void Keypad::Init(BYTE mode)
{
	...
	else if (mode == KEYPAD_INT_MODE)
	{ // KEYPAD_INT_MODE
		// Insert interrupt init here for exercise 3
		/* set up external interrupt input pin with interrupt service routine */
		SetPinIrq(D_AVAIL_PIN, RISING_EDGE, &EdgePortISR1);
		/* initialize myQueue for communication between interrupt service routine and main task */
...
void Keypad::read_data(void)
{
	...
	last_encoded_data = 0;
	if (KEYPAD_DO_A)
		last_encoded_data += 0x01;
	if (KEYPAD_DO_B)
		last_encoded_data += 0x02;
	if (KEYPAD_DO_C)
		last_encoded_data += 0x04;
	if (KEYPAD_DO_D)
		last_encoded_data += 0x08;

	if (byte_mode == KEYPAD_INT_MODE)
	{
		// Insert exercise 3 code here
		/* restore back for B side */
		KEYPAD_MUX_CTRL.set();				 // back to B side
		KEYPAD_DO_D.function(PINJ2_45_IRQ1); // change back to IRQ(interrupt)
		KEYPAD_DO_D.drive();				 // turn output on
		/* enable interrupt */
		EnableIrq(D_AVAIL_PIN);
	}
}
...
void Keypad::EdgePortISR1(void)
{
	...
	/* get the pointer to the string representing the pressed button */
	const char *button_pressed = myKeypad.GetNewButtonString();
	iprintf("post: %s\n", button_pressed); // echo this string to console for exercise 3
	/* post this data about the pressed button to myQueue */
	OSQPost(&myQueue, (void *)button_pressed);
}
